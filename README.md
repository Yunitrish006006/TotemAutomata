...
目前版本需要 TotemCore `>=0.7.21 <0.8.0`；除了 Automata 自有 production
...
2. TotemCore `0.7.18`（支援 `>=0.7.21 <0.8.0`）
...
- 銅魁儡與容器之間改用 TotemCore 共用實線：可用來源為橘色、可用目的地
  為綠色、不可用容器為紅色。線條起點會在每個渲染幀跟隨客戶端銅魁儡的
  插值位置，容器關係仍由伺服器權威快照決定。這些線同樣會被地形遮住，
  不會穿牆；目前採集目標與阻塞狀態仍保留原本的粒子提示。
- 每個目的地的手動篩選會各顯示 8 個「允許」與「拒絕」規則；超過時可在
  左右清單上分別滾動，每次移動一列，底部會顯示目前範圍與總數。
