...
public final class CopperGolemActivityResolver {
    ...
    /**
     * Returns the activity that the pre-split Wrench menu displayed and writes
     * it back to the shared persisted tag, matching the existing snapshot
     * side effect.
     */
    ...
    public static CopperGolemActivity resolve(CopperGolem golem, ServerLevel level, CompoundTag tag) {
        ...
        if (SortingBindingService.getSourceContainer(tag).isEmpty()) {
            return CopperGolemActivity.BLOCKED_NO_HOME;
        }
        if (CopperGolemData.mode(tag) == CopperGolemMode.SORTING) {
            if (tag.getBooleanOr(SORTING_BLOCKED, false)) {
                return CopperGolemActivity.BLOCKED_SORTING;
            }
            if (golem.getMainHandItem().isEmpty() && !CopperGolemFuelService.hasFuelAvailable(tag, level)) {
        ...
        if (!storage.isEmpty()) {
            ...
            if (GatheringStorage.full(storage)) {
        ...
        if (!CopperGolemFuelService.hasFuelAvailable(tag, level)) {
            return CopperGolemActivity.BLOCKED_NO_FUEL;
        }
        if (GatheringConfiguration.scanBounds(tag, level.dimension()).isEmpty()) {
            return CopperGolemActivity.BLOCKED_NO_AREA;
        }
        if (!hasTargetRules(tag)) {
        ...
        return GatheringRuntimeState.target(tag)
                .map(target -> isAtGatheringTarget(golem, target)
    ...
    private static boolean hasTargetRules(CompoundTag tag) {
        return !GatheringConfiguration.manualTargets(tag).isEmpty()
                || GatheringLlmState.read(tag).usable(GolemLlmState.read(tag));
    }

    /** Preserved mining reach rule for gathering display and runtime activity. */
    public static boolean isAtGatheringTarget(CopperGolem golem, BlockPos pos) {
        ...
        if (upwardY >= 1 && upwardY <= 2 && upwardX <= 1 && upwardZ <= 1) {
            return true;
        }
