...
public final class DefaultGatheringWorldOperations implements PersistedGatheringBehavior.WorldOperations {
    ...
    @Override public boolean isValidTarget(CopperGolem golem, ServerLevel level, CompoundTag tag, BlockPos pos) {
        ...
        ItemStack tool=CopperGolemData.readItemStack(tag,TOOL,level.registryAccess()); var state=level.getBlockState(pos); var drops=GatheringDrops.resolve(golem,level,pos,state,tool); if(drops.isEmpty()||!GatheringStorage.canStore(CopperGolemData.readItemStack(tag,STORAGE,level.registryAccess()),drops.get()))return false;
